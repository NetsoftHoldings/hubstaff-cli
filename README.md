...
| 5 | Token validity | token is fresh, or near-expiry with a successful refresh, or expired with a successful refresh | near expiry (within 300 s) **and** no refresh token; or no `expires_at` at all — a long-lived token is expected and needs nothing, while a session with a refresh token has lost proactive refresh | expired without refresh token; or refresh attempt failed | no credentials | `hubstaff config set-pat <TOKEN>` (not offered for a long-lived token, which needs no remediation) |
